...
Public Class frmCreditAccount
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        If txtAccount.Text = "" Then
            MsgBox("Please enter credit account name", vbCritical, Me.Text)
            txtAccount.Focus()
            Exit Sub
        End If
        If mode.Text = "edit" Then
            com.CommandText = "update tblcreditaccount set accountname='" & rchar(txtAccount.Text) & "' where acctnumber='" & acctnumber.Text & "'" : com.ExecuteNonQuery()
        Else
            com.CommandText = "insert into tblcreditaccount set acctnumber='" & acctnumber.Text & "',accountname='" & rchar(txtAccount.Text) & "'" : com.ExecuteNonQuery()
        End If
    ...
    Public Sub FilterItem()
        Try
            ...
            msda = New MySqlDataAdapter("select acctnumber,accountname as 'Account Name' from tblcreditaccount order by accountname asc", conn)
            msda.Fill(dst, 0)
            Em.DataSource = dst.Tables(0)
            With Em
                .Columns("acctnumber").Visible = False
            End With
            acctnumber.Text = codeGenerator("tblcreditaccount", "acctnumber", 6, "CRDT-", "100001")
    ...
    Private Sub EditChapterToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles EditChapterToolStripMenuItem.Click
        com.CommandText = "select * from tblcreditaccount where acctnumber='" & Em.Item("acctnumber", Em.CurrentRow.Index).Value() & "'" : rst = com.ExecuteReader
        While rst.Read
    ...
    Private Sub TagAsInactiveMemberToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles TagAsInactiveMemberToolStripMenuItem.Click
        If MessageBox.Show("Are you sure you want to delete selected item?", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) = vbYes Then
            com.CommandText = "delete from tblcreditaccount where acctnumber='" & Em.Item("acctnumber", Em.CurrentRow.Index).Value() & "'" : com.ExecuteNonQuery()
            FilterItem()
